...
As established in the [protocol] page, you'll first need to decide on an "Application-Layer Protocol Negotiation" (ALPN) string.
...
This implements a simple [request-response pattern](/protocols/using-quic#request-and-response):
...
We've [written about it][closing connections] before, but it trips us up every now and then still. For detailed guidance on closing connections, see the [Closing Connections](/protocols/using-quic#closing-connections) section of the QUIC guide.
</Note>
...
This follows the [request-response pattern](/protocols/using-quic#request-and-response) from the connecting side:
...
Note that we also take a `EndpointAddr` as a parameter.
...
async fn main() -> Result<()> {
    ...
    let endpoint_addr = router.endpoint().addr().await?;
...
- **Re-use connections**: The version of the echo protocol above simply closes the connection after having echo-ed one stream.
  ...
  For examples of handling multiple requests on the same connection, see [Multiple Requests & Responses](/protocols/using-quic#multiple-requests--responses).
- **Beware: QUIC streams are lazy**: Make sure that when you call `connection.open_bi()`, you *always send first* before you receive data.
  ...
  Learn more about streams in the [Overview of the QUIC API](/protocols/using-quic#overview-of-the-quic-api).
- **Closing QUIC connections can be hard**: This was already mentioned above, but it's worth re-iterating.
  ...
  For everything else, feel free to read our blog post about [closing connections] or the detailed guide on [Closing Connections](/protocols/using-quic#closing-connections).
